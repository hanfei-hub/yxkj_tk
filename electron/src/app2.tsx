...
function SmartSelection({ user, onNotice, onAddLibrary }: { user: User; onNotice: (message: string) => void; onAddLibrary: (p: Product) => Promise<void> }) {
  const [message, setMessage] = useState("");
  const [taskId, setTaskId] = useState<number | null>(null);
  const [progress, setProgress] = useState(0);
