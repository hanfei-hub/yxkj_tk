...
import "./smart-selection-report.css";
...
function ProductGrid({ items, compact = false, report = false }: { items: Item[]; compact?: boolean; report?: boolean }) {
  if (!items.length) return <div className="selection-stage-wait">当前阶段暂无可展示商品。</div>;
  const gridClass = report ? "selection-report-product-grid" : compact ? "selection-stage-product-grid compact" : "selection-stage-product-grid";
  const cardClass = report ? "selection-report-product" : "selection-stage-product";
  return <div className={gridClass}>{items.slice(0, compact ? 8 : 10).map((item, index) => <article className={`${cardClass} ${item.eliminated ? "eliminated" : ""}`} key={itemKey(item, index)}><div>{itemImage(item) ? <img src={itemImage(item)} loading="lazy" /> : <span>暂无图片</span>}</div><section><b>{itemTitle(item)}</b><small>{item.eliminated ? "已淘汰" : `销量 ${Number(item.sales_count || 0).toLocaleString()}`}</small></section></article>)}</div>;
}

function SelectionReportModal({ task, onClose, onExport }: { task: PipelineTask; onClose: () => void; onExport: () => void }) {
  const analysis = (task.ai_report || {}) as Record<string, any>;
  const market = (analysis.market_intro || {}) as Record<string, unknown>;
  const positioning = (analysis.product_positioning || {}) as Record<string, unknown>;
  const audience = (analysis.target_audience || {}) as Record<string, unknown>;
  const tracks = Array.isArray(analysis.opportunity_tracks) ? analysis.opportunity_tracks as Array<Record<string, unknown>> : [];
  const restrictions = asItems(analysis.restriction_flags);
  const risks = Array.isArray(analysis.risk_advice) ? analysis.risk_advice : [];
  const finals = asItems(task.final_items);
  const candidates = asItems(task.candidate_items);
  const products = finals.length ? finals : candidates;
  const tiers = finals.reduce<Record<string, number>>((value, item) => { const key = String(item.tier || "regular"); value[key] = (value[key] || 0) + 1; return value; }, {});
  useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); }; window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close); }, [onClose]);
  return <div className="selection-report-modal" role="dialog" aria-modal="true" aria-label="智能选品市场机会报告"><div className="selection-report-mask" onClick={onClose} /><section className="selection-report-dialog"><header><div><span>已生成</span><h2>智能选品市场机会报告</h2><p>任务 #{task.id || "-"} · 日本 TikTok 蓝海选品</p></div><div className="selection-report-actions"><button className="secondary" onClick={onExport}>导出 PDF</button><button className="selection-report-close" onClick={onClose} aria-label="关闭报告">×</button></div></header><main>
    <section className="selection-report-section"><h3>市场简介</h3><div className="selection-report-facts"><article><b>市场规模</b><p>{String(market.market_scale || "需进一步验证")}</p></article><article><b>需求趋势</b><p>{String(market.demand_trend || "需进一步验证")}</p></article><article><b>竞争格局</b><p>{String(market.competition_landscape || "需进一步验证")}</p></article></div></section>
    <section className="selection-report-section"><h3>产品定位画像</h3><div className="selection-report-facts"><article><b>定位描述</b><p>{String(positioning.positioning || "需进一步验证")}</p></article><article><b>核心卖点</b><p>{String(positioning.selling_point || "需进一步验证")}</p></article><article><b>风格调性</b><p>{String(positioning.style || "需进一步验证")}</p></article></div></section>
    <section className="selection-report-section"><h3>目标受众分析</h3><div className="selection-report-facts"><article><b>人群画像</b><p>{String(audience.persona || "需进一步验证")}</p></article><article><b>痛点需求</b><p>{String(audience.pain_point || "需进一步验证")}</p></article><article><b>使用场景</b><p>{String(audience.scene || "需进一步验证")}</p></article></div></section>
    <section className="selection-report-section"><h3>机会赛道与可切入商品</h3>{tracks.length ? tracks.map((track, index) => <div className="selection-report-track" key={`${track.track_name || "track"}-${index}`}><h4>{String(track.track_name || "核心机会赛道")}</h4><p>{String(track.opportunity || track.reason || "基于本次筛选结果，建议继续验证市场机会。")}</p><ProductGrid items={products.slice(index * 2, index * 2 + 2)} compact report /></div>) : <ProductGrid items={products.slice(0, 6)} compact report />}</section>
    <section className="selection-report-section"><h3>精选组合概览</h3><p className="selection-report-copy">最终精选商品按爆款、常规、新品、奇特与亮点等层次搭配，兼顾已验证需求和内容出圈潜力。</p><div className="selection-report-tiers">{["hot", "regular", "new", "quirky", "highlight"].map((tier) => <span key={tier}>{({ hot: "爆款", regular: "常规", new: "新品", quirky: "奇特", highlight: "亮点" } as Record<string, string>)[tier]} × {tiers[tier] || 0}</span>)}</div><ProductGrid items={finals} compact report /></section>
    <section className="selection-report-section"><h3>平台限售与风险提示</h3>{restrictions.length ? <ul>{restrictions.map((item, index) => <li key={itemKey(item, index)}>{String(item.desc || item.reason || item.category || JSON.stringify(item))}</li>)}</ul> : <p className="selection-report-copy">本次候选及精选商品未命中已配置的日本限售规则；上架前仍需复核商品标签、认证与平台规则。</p>}{risks.length ? <ul>{risks.map((risk, index) => <li key={index}>{String(risk)}</li>)}</ul> : null}</section>
    <section className="selection-report-section"><h3>行动建议</h3><p className="selection-report-copy"><b>候选策略：</b>{String(analysis.candidate_strategy || "优先验证候选商品的供应链稳定性、成本和短视频素材表现。")}</p><p className="selection-report-copy"><b>精选策略：</b>{String(analysis.final_strategy || "上架前完成法规、限售和专利风险复核。")}</p><p className="selection-report-conclusion">{String(analysis.conclusion || "请结合本次商品清单进行人工复核与小批量测试。")}</p></section>
  </main></section></div>;
}

function SelectionTimeline({ status, currentStep, keywords, groups, counters, boardItems, reportTask, tick, onOpenReport }: { status: string; currentStep: number; keywords: Item[]; groups: BoardGroup[]; counters: Record<string, number>; boardItems: Item[]; reportTask: PipelineTask | null; tick: number; onOpenReport: () => void }) {
  ...
  const stageBody = (index: number) => {
    ...
    if (index === 4) return reportTask ? <section className="selection-report-preview"><h3>精品分析报告</h3><p>{String(analysis.conclusion || analysis.market_summary || "市场分析已生成，可查看完整报告。")}</p><div><span><b>市场规模</b>{String(market.market_scale || "待进一步验证")}</span><span><b>需求趋势</b>{String(market.demand_trend || "待进一步验证")}</span><span><b>竞争格局</b>{String(market.competition_landscape || "待进一步验证")}</span></div><button className="secondary" onClick={onOpenReport}>查看完整市场机会报告</button></section> : <div className="selection-stage-wait">正在整理市场机会、风险和用户需求分析…</div>;
