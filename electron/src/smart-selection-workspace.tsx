...
export function SmartSelectionWorkspace({ user, onNotice }: { user: User; onNotice: (message: string) => void }) {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      setStatus("running"); setStage("created"); setProgress(1); setTaskMessage("正在创建智能选品任务"); setGroups([]); setCounters({}); setLogs([]);
  ...
  async function exportReport() {
    ...
    const reportTaskId = taskId || Number(localStorage.getItem(storageKey) || 0);
    if (!reportTaskId) return;
    try {
      const payload = await service.getSelectionPipelineReportContent(reportTaskId) as PipelineTask;
      const candidates = asItems(payload.candidates);
      const finals = asItems(payload.finals);
      const analysis = payload.analysis || {};
      const renderRows = (items: Array<Record<string, unknown>>) => items.map((item, index) => `<tr><td>${index + 1}</td><td>${itemTitle(item)}</td><td>¥${Number(item.price || 0).toFixed(2)}</td><td>${Number(item.sales_count || 0).toLocaleString()}</td></tr>`).join("");
      const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>智能选品报告</title><style>body{font-family:"Microsoft YaHei",sans-serif;color:#23364a;margin:38px;line-height:1.65}h1{margin:0 0 8px}h2{margin-top:28px;color:#117b62}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:8px;border:1px solid #dbe4e9;text-align:left}th{background:#eef8f4}.note{color:#65798b;white-space:pre-wrap}</style><h1>智能选品市场机会报告</h1><p class="note">任务 #${reportTaskId}</p><h2>市场分析</h2><p>${String(analysis.market_summary || "本次任务完成后，系统根据商品和市场数据生成选品判断。")}</p><h2>候选蓝海商品</h2><table><tr><th>#</th><th>商品</th><th>价格</th><th>EchoTik 销量</th></tr>${renderRows(candidates)}</table><h2>最终精选商品</h2><table><tr><th>#</th><th>商品</th><th>价格</th><th>EchoTik 销量</th></tr>${renderRows(finals)}</table></html>`;
      if (window.desktop?.savePdf) {
        const result = await window.desktop.savePdf(html, `智能选品报告-${reportTaskId}.pdf`);
        if (result.canceled) return;
      } else {
        const blob = new Blob([html], { type: "text/html;charset=utf-8" });
        const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `智能选品报告-${reportTaskId}.html`; link.click(); URL.revokeObjectURL(link.href);
      }
      onNotice("选品报告已下载");
